...
void printRBTRec(const std::string& prefix, const RBTNode* node, bool isLeft)
{
	if( node != nullptr )
	{
		...
		printRBTRec( prefix + (isLeft ? "│   " : "    "), node->left_child, true);
		printRBTRec( prefix + (isLeft ? "│   " : "    "), node->right_child, false);
	}
}
...
void DoubleRedIfSIsBlackLeft( RBTNode *b) {
	RBTNode *a;
	RBTNode *c;
	int tmp_key;
...
void DoubleRedIfSIsBlackRight( RBTNode *b ) {
	RBTNode *a;
	RBTNode *c;
	int tmp_key;
...
void DoubleRedIfSIsRedLeft( RBTNode *y ) {
	RBTNode *z;
	RBTNode *s;
	...
	if (!z->parent)
		...
		DoubleRedIfSIsRedLeft(z);
	}
	std::cout << "Heee" << std::endl;
}

void DoubleRedIfSIsRedRight( RBTNode *y ) {
	RBTNode *z;
	RBTNode *s;
	...
	else
	{
		...
		if (!leaf->is_black) {
			if (!leaf->parent->right_child || leaf->parent->right_child->is_black )
				DoubleRedIfSIsBlackLeft(leaf);
			else
				DoubleRedIfSIsRedLeft(leaf);
		}
	}  
  }
	...
	else
	{
		...
		if (!leaf->is_black) {
			if (!leaf->parent->left_child || leaf->parent->left_child->is_black)
				DoubleRedIfSIsBlackRight(leaf);
			else 
				DoubleRedIfSIsRedRight(leaf);
		}
	}
  }
}
