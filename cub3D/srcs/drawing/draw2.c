...
/*   Updated: 2021/03/24 19:46:45 by tpetit           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	t_image *texture, float y_value)
{
	...
	int			color;
	const int	offset = (c_ray->screen_h - x_len[1]) / 2 + c_ray->look_offset;
	const int	texture_y = texture->height * y_value;

	i = -1;
	while (++i < x_len[1] && i + offset < c_ray->screen_h)
	{
		get_pixel(texture, texture_y, i / texture_ratio, &color);
		if (i + offset > 0)
